...
def random_full_name(df_names: np.ndarray, df_surnames: np.ndarray) -> str:
    i = random.randint(0, len(df_names) - 1)
    j = random.randint(0, len(df_surnames) - 1)

    return f"{df_names[i][0]} {df_surnames[j][0]}"


def random_color() -> str:
    colors = np.array(['gold', 'silver', 'blue', 'brown', 'yellow', 'teal', 'magenta', 'purple', 'olive', 'black', 'white', 'gray', 'red','green'])
...
def random_registration_plate() -> str:
    ...
    registration_plate += random.choice(string.ascii_uppercase)
    registration_plate += str(random.randint(0, 5))
    registration_plate += str(random.randint(0, 9))
...
def random_mobile_phone_number() -> str:
    mobile_phone_number = "+44 20"
    mobile_phone_number += str(random.randint(70, 79))

    for i in range(0, 4):
        mobile_phone_number += str(random.randint(10, 99))
...
def random_email(full_name: str, date_of_birth: str = None) -> str:
    domains = np.array(["gmail.com", "yandex.ru", "ukr.net"])
    domain = domains[random.randint(0, len(domains) - 1)]
    email_format = random.randint(1, 4)
    ...
    if email_format == 1:
        ...
        result = full_name.replace(" ", "_").lower()
        if date_of_birth is not None:
            result += date_of_birth.split("-")[0]
        result += "@" + domain
        return result
    elif email_format == 4:
        result = full_name.split(" ")[0].lower()
        if date_of_birth is not None:
            result += "_" + date_of_birth.split("-")[0]
        result += "@" + domain
        return result
...
def random_client_feedback() -> List[str]:
    ...
    return list([categories[i] for i in range(random.randint(1, 4), random.randint(5, len(categories) - 1))])


def random_client_comment(lib: np.ndarray, rating: int = None) -> str:
    if rating is not None:
        result = [lib[i][1] for i in np.where(lib[:, 0] == rating)[0]]
    else:
        result = [lib[i][1] for i in range(0, len(lib))]
...
def calculate_cost(distance: float, ride_time: float) -> float:
    return round(2 * distance + 0.75 * ride_time, 2)
...
def generate_drivers(count: int) -> None:
    names = pd.read_csv(NAMES_CSV).to_numpy()[0:250]
    surnames = pd.read_csv(SURNAMES_CSV).to_numpy()[0:250]
    connection = open_connection()
    ...
    for i in range(0, count):
        ...
        if i % 2 == 0:
            ...
            date_of_birth = random_date(1950, 1999)
...
def generate_clients(count: int) -> None:
    names = pd.read_csv(NAMES_CSV).to_numpy()[250:500]
    surnames = pd.read_csv(SURNAMES_CSV).to_numpy()[250:500]
    connection = open_connection()
    locations = list(connection.london.postcodes.aggregate([{'$sample': {'size': count}}]))
    connection.close()
...
if __name__ == '__main__':
    print("\nGenerate drivers:\n")
    generate_drivers(count=1000)
    print("\nGenerate clients:\n")
    generate_clients(count=1500)
    print("\nGenerate routes:\n")
